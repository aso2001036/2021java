...
public class RPGMain {
    ...
    private boolean battle(){
        ...
        if( !monster.isAlive() ){
            ...
            okane = okane + monster.getMoney();
        }
        
        //3体居なくなった？
        if( isNotThereAllMonster() ){
            System.out.println(okane+"gold獲得");
            //すべて居なくなったら終了
            return false;
        }
        ...
        for(int i=0; i < MONSTER_NUM;i++){
            if(!monsters[i].isThere()){
                System.out.println(monsters[i].getName()+"は死んでる");
            }else{
